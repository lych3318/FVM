...
class LVM2(LVM):
	...
	# lvcreate --snapshot -L 200m -n snap /dev/mapper/xxx
	def lv_create_snapshot(self, origindev, lvname, size):
		#vg = self.get_vgname(vgname)
		print 'lv_create_snapshot------------------------------'
		unit = size[-1:]
		if unit in LVM.SIZE_UNITS or unit in LVM.EXTENDS_UNITS:
			# lv create command
			argv = list()
			...
			argv.append(lvname)
			#argv.append(vg.name)
			argv.append(origindev)
			(status, output) = process_call_argv(argv)
			print output
			if status != 0:
				print 'lv_create_snapshot failed---------------------'
				logging.debug('error creating LV')
				return output
			logging.info(output)
			return None
